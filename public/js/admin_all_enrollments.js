(() => {
    ...
    function downloadToClientExcel(file, name, e = null, name_button) {
        try{
            const url = window.URL.createObjectURL(new Blob([file]));
            const link = document.createElement('a');
            link.href = url;
    ...
    async function downloadStudentExcel(e) {
        ...
        try {
            let [enrollment_id, no_identifier, period, year] = JSON.parse(e.target.dataset.excel),
                response = await fetch(`/admin/all_enrollments/${enrollment_id}`)
            if (response.status == 200) {
                let file = await response.blob()
    ...
    async function downloadAllEnrollmentsExcel(e) {
        ...
        try {
            if(filters.length > 0){
                ...
                response = await fetch(`/admin/all_enrollments`, options)
                if (response.status == 200) {
                    let file = await response.blob(),
                        name = `${filters.map( f => {
    ...
    function deleteEnrollment(e){
        ...
        try{
            ...
            socket.emit('delete_enrollment_admin', e.target.dataset._id)
        }
        catch(e){
    ...
    socket.on('delete_enrollment_admin_success', recharge)
    socket.on('delete_enrollment_admin_error', message => {
