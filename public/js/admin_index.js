(function () {
    ...
    function processSubmit(e) {
        try {
            ...
            if (e.target.checkValidity()) {
                ...
                e.target.querySelector('button[type="submit"]').innerHTML = spinner('Calificando')
                e.target.querySelector('button[type="submit"]').setAttribute('disabled', '')
                socket.emit('admin_qualifies', data)
            }
            e.target.classList.add('was-validated')
        } catch (err) {
            e.target.querySelector('button[type="submit"]').innerHTML = 'Confirmar'
    ...
    socket.on('admin_qualifies_error', mesg => {
        message_area.innerHTML = getMessage(false, mesg)
    })

    socket.on('admin_qualifies_success', id => {
        try {
    ...
    socket.on('enroll_qualified', _id => {
        try {
